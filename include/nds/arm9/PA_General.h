...
#include <stdarg.h> // Pour le syst�me de text
#include <string.h>
...
extern s16 PA_ScreenSpace; // Espace entre les 2 �crans...+192
...
// Commandes pour la lumi�re des �crans
...
typedef struct {
   ...
   u8 Message[53];
   u8 MessageLength;
   u8 Language; // Langue... 0 pour Jap, 1 pour anglais, 2 pour fran�ais...
} infos;
...
/*! \fn void PA_Init(void)
    \brief
         ...
         \~french Initialise la lib... Doit etre plac� au d�but de main()
*/
void PA_Init();
...
/*! \fn void PA_InitFifo()
    \brief
         ...
         \~french Initialise le syst�me Fifo. C'est fait automatiquement dans PA_Init().
*/
void PA_InitFifo();
...
/*! \fn void PA_Init2D(void)
    \brief
         ...
         \~french Remet en mode 2D apr�s avoir utilis� la 3D
*/
void PA_Init2D();
...
/*! \fn void PA_SetVideoMode(u8 screen, u8 mode)
    \brief
         ...
         \~french Changer de mode video... A utiliser avec pr�caution
    \~\param screen
         \~english Screen...
         \~french Ecran...
    \~\param mode
...
/*! \fn void PA_UpdateUserInfo(void)
    \brief
         ...
         \~french Met � jour les infos sur l'utilisateur... C'est fait automatiquement dans PA_Init. On peut ensuite r�cup�rer toutes les infos avec PA_UserInfo.Color (couleur favorite), .BdayDay, .BdayMonth, .AlarmHour, .AlarmMinute, .Name, .NameLength, .Message, .MessageLength, .Language
*/
...
/*! \fn void PA_UpdateRTC(void)
    \brief
         ...
         \~french Met � jour les infos sur l'heure et la date. C'est automatiquement mis � jour dans le VBL PA... On r�cup�re les infos avec PA_RTC.Minutes, .Hour, .Seconds, .Day, .Month, et .Year
*/
...
/*! \fn static inline void PA_SwitchScreens()
    \brief
         ...
         \~french Echange les �crans du haut et du bas
*/
...
/*! \def PA_LidClosed()
    \brief
         ...
         \~french V�rifie si la DS est ferm�e. Renvoie 0 si ouverte, 1 si ferm�e
*/
...
/*! \fn static inline void PA_SetAutoCheckLid(u8 on)
    \brief
         ...
         \~french V�rifie automatiquement si la DS est ferm�e dans PA_WaitForVBL
    \~\param on
         \~english 1 for on, 0 for off
         \~french 1 pour activer, 0 pour d�sactiver
*/
...
/*! \fn u8 PA_CheckLid()
    \brief
         ...
         \~french V�rifie si la DS est ferm�e. Si ferm�e, ca met en pause la DS et renvoie 1.
*/
u8 PA_CheckLid();
...
/*! \def PA_CloseLidSound(close_sound)
    \brief
         ...
         \~french V�rifie si la DS est ferm�e. Si ferm�e, ca met en pause la DS et joue un son
    \~\param close_sound
         \~english Sound to play, check the sounds doc if you're not sure what to do here
         \~french Son � jouer, regarder la doc son si pas certain de quoi mettre... 
*/
...
/*! \def PA_CloseLidSound2(close_sound, open_sound)
    ...
    \~\param close_sound
         ...
         \~french Son � jouer quand se ferme, regarder la doc son si pas certain de quoi mettre...
    \~\param open_sound
         \~english Sound to play when opens, check the sounds doc if you're not sure what to do here
         \~french Son � jouer quand s'ouvre, regarder la doc son si pas certain de quoi mettre... 
*/
...
/*! \fn void PA_SetScreenLight(u8 screen, u8 light)
    \brief
         ...
         \~french Allumer ou eteindre la lumi�re d'un �cran
    \~\param screen
         \~english Screen...
         \~french Ecran...
    \~\param light
         \~english Light, 1 for on, 0 for off
         \~french Lumi�re, 1 pour allum�, 0 pour �teint
*/
...
/*! \fn static inline void PA_SetLedBlink(u8 blink, u8 speed)
    \brief
         ...
         \~french R�gler le clignotement de la led
    \~\param blink
         \~english 1 for blinking, 0 for always on
         \~french 1 pour clignoter, 0 pour toujours allum�
    \~\param speed
...
/*! \def PA_WaitFor(something)
    \brief
         ...
         \~french Attendre un �v�nement pr�cis...
    \~\param something
         \~english Thing to wait for, like Pad.Newpress.A, or Stylus.Newpress, etc...
         \~french Ev�nement � attendre, comme Pad.Newpress.A, ou Stylus.Newpress, etc...
*/
...
/*! \fn static inline void PA_SetDSLBrightness(u8 level)
    \brief
         ...
         \~french R�gler le niveau de lumi�re de la DS Lite !
    \~\param level
         \~english Light level (0-3)
         \~french Niveau de la lumi�re (0-3)
*/
